import CloseIcon from '@mui/icons-material/Close'
import {
	Button,
	Dialog,
	DialogContent,
	DialogTitle,
	IconButton,
	Table,
	TableBody,
	TableCell,
	TableHead,
	TableRow,
	useTheme,
} from '@mui/material'
import { useState } from 'react'
import { InvestmentIdicators } from '../../../../types/investment.interface'
import arrowUp from '/svg/Polygon 2 (1).svg'
import arrowDown from '/svg/Polygon 2.svg'

interface Props {
	indicators: InvestmentIdicators[]
}

function IndicatorsModal({ indicators }: Props) {
	const [open, setOpen] = useState(false)
	const handleOpen = () => setOpen(true)
	const handleClose = () => setOpen(false)
	const theme = useTheme()
	return (
		<>
			<Button
				variant='text'
				sx={{
					alignSelf: 'end',
					mt: 1,
					fontWeight: 'bold',
					color: 'primary.main',
				}}
				onClick={() => handleOpen()}
			>
				Показать все →
			</Button>

			<Dialog
				open={open}
				onClose={handleClose}
				maxWidth='md'
				fullWidth
				PaperProps={{
					sx: {
						backgroundColor: theme.palette.background.default,
					},
				}}
			>
				<DialogTitle sx={{ m: 0, p: 2 }}>
					Полный список регионов
					<IconButton
						aria-label='close'
						onClick={handleClose}
						sx={{
							position: 'absolute',
							right: 8,
							top: 8,
							color: theme.palette.grey[500],
						}}
					>
						<CloseIcon />
					</IconButton>
				</DialogTitle>
				<DialogContent dividers>
					<Table>
						<TableHead>
							<TableRow>
								<TableCell>
									<strong>Регион</strong>
								</TableCell>
								<TableCell>
									<strong>Объём</strong>
								</TableCell>
								<TableCell>
									<strong>Изменение</strong>
								</TableCell>
								<TableCell>
									<strong>%</strong>
								</TableCell>
							</TableRow>
						</TableHead>
						<TableBody>
							{indicators.map((item, index) => (
								<TableRow key={index}>
									<TableCell>{item.region_name}</TableCell>
									<TableCell>{item.value} T</TableCell>
									<TableCell>
										<img
											src={item.percent.status === 'up' ? arrowUp : arrowDown}
											alt={item.percent.status}
											width={12}
										/>
									</TableCell>
									<TableCell>{item.percent.value}%</TableCell>
								</TableRow>
							))}
						</TableBody>
					</Table>
				</DialogContent>
			</Dialog>
		</>
	)
}

export default IndicatorsModal
